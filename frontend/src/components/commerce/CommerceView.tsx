  ...
  return (
    <div className="flex-1 flex flex-col bg-black min-h-0">
      ...
      <div className="px-4 py-3 border-b border-rally-border bg-[#0D1117]/80 flex items-center gap-3">
        ...
        <h2 className="font-display font-bold text-rally-text">Server Shop</h2>
        ...
        {PRODUCT_TYPES.map((t) => (
          <button
            ...
            className={cn(
              ...
              filter === t.id ? 'bg-rally-green/10 text-rally-green border border-rally-green/30' : 'bg-white/5 text-rally-text-muted hover:bg-white/10'
            )}
          >
      ...
      <div className="flex-1 overflow-y-auto p-4">
        {isLoading ? (
          ...
          <div className="text-center py-12">
            ...
            <p className="text-rally-text-muted">No products available yet.</p>
          </div>
        ) : (
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
            {filtered.map((product) => (
              <button key={product.id} onClick={() => setSelectedProduct(product)} className="card-rally rounded-lg p-3 text-left hover:border-rally-green/30 transition-all">
                ...
                <p className="text-xs text-rally-text-muted truncate">{product.description || 'No description'}</p>
      ...
      {selectedProduct && (
        <Modal isOpen onClose={() => setSelectedProduct(null)} title={selectedProduct.title} size="md">
          <div className="space-y-4">
            ...
            <p className="text-sm text-rally-text-muted">{selectedProduct.description || 'No description provided.'}</p>
            <div className="flex items-center justify-between">
              <div>
                <p className="text-xs text-rally-text-muted">Price</p>
              ...
              <div className="text-right">
                <p className="text-xs text-rally-text-muted">Seller</p>
            ...
            <button onClick={() => handlePurchase(selectedProduct)} className="btn-rally-primary w-full py-3">
              <DollarSign className="w-4 h-4 inline mr-1" />Purchase
            </button>
          </div>
        </Modal>
      )}
